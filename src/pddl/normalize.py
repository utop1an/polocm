...
def all_conditions(task):
    """
    Build the generator of all the conditions' proxy/copy for a task
    """
...
def normalize(task):
    """
    Call this function to normalize a task.

    The origin task may contain complicated conditions and effects that we don't desire, e.g. implication, quantifiers.
    Logical equivalences are applied to simplify the task, i.e. reconstruct the conditions and effects in a uniform way
    that we want to use latter.

    Accordingly, the normalized task should be:
        – The goal formula is a conjunction of literals.
        – All axiom bodies are conjunctions of literals (except for the possible implicit
          existential quantification of free variables not occurring in the axiom head).
        – All operator preconditions are conjunctions of literals.
        – All effect conditions are conjunctions of literals.
        – All operator effects are conjunctions of universally quantified conditional
          simple effects

    more on: http://cs.uky.edu/~sgware/reading/papers/helmert2008translation.pdf
    """
