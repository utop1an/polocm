...
def single(obs_po_tracelist ,obs_tracelist: ObservedTraceList, domain_filename, output_dir ,logger, verbose=False):
    try: 
        remark = []
        model, AP, runtime = POLOCMBASELINE(obs_po_tracelist, sort_knowledge=False, cores=CT, logger=logger )
        ...
        sorts = POLOCMBASELINE._get_sorts(obs_tracelist)
        AML, _, __ = POLOCMBASELINE._locm2_step1(obs_tracelist, sorts)
...
def single_locm2(obs_tracelist: ObservedTraceList, domain_filename, output_dir, verbose=False, logger=None):
    try: 
        remark = []
        model,_, runtime = POLOCMBASELINE(obs_tracelist, sort_knowledge=False, logger=logger)
        filename = domain_filename + ".pddl"
        file_path = os.path.join(output_dir, "pddl", filename)
        tmp_file_path = os.path.join(output_dir, "pddl", "tmp", filename)
        model.to_pddl(domain_filename, domain_filename=file_path, problem_filename=tmp_file_path)
...
def get_AP_accuracy(learned_AML, AML, verbose=False):
    if (len(learned_AML)==0):
        return 0,1, "learned_AML Empty"
    if (len(learned_AML) != len(AML)):
        return 0,1, "learned_AML Invalid Length"
    acc = []
    err = []
    for sort, m1 in enumerate(learned_AML):
