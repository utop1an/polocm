...
class IAP:
    ...
    def toAP(self):
        if self.pos != None:
            ...
            raise Exception("Cannot convert IAP to AP without a position")
...
TSType = Dict[FSM, Dict[PlanningObject, List[tuple]]]  # {FSM: {obj: [AP]}}
...
class POLOCMBASELINE:
    ...
    def __new__(
        ...
        if (not sort_knowledge):
            sorts = POLOCMBASELINE._get_sorts(obs_tracelist, debug["sorts"])
            sort_lookup = None
        else:
            sorts, sort_lookup = POLOCMBASELINE._get_sorts_from_knowledge(obs_tracelist)
        ...
        return POLOCMBASELINE.baseline(sorts, obs_tracelist, debug, sort_lookup)

    @staticmethod
    @set_timer_throw_exc(num_seconds=600, exception=POLOCMTimeOut, max_time=600, stage='polocm')
    def baseline(sorts, obs_tracelist, debug, sort_lookup = None):
        try:
            start = time.time()
            obj_trace_PO_matrix_overall, dependencies = POLOCMBASELINE.polocm_step1(obs_tracelist, sorts, debug['pstep1'])
            obj_consecutive_transitions_overall = POLOCMBASELINE.polocm_step2(obj_trace_PO_matrix_overall, debug['pstep2'])
            polocm_time = time.time() - start

            AML = POLOCMBASELINE._locm2_step0(obj_consecutive_transitions_overall, sorts, debug['2step0'])
            AML_with_holes = POLOCMBASELINE._locm2_step2(AML, debug['2step2'])
            H_per_sort = POLOCMBASELINE._locm2_step3(AML_with_holes, debug['3step3'])
            transitions_per_sort = POLOCMBASELINE._locm2_step4(AML_with_holes)
            consecutive_transitions_per_sort = POLOCMBASELINE._locm2_step5(AML_with_holes)
            S = POLOCMBASELINE._locm2_step6(AML, H_per_sort, transitions_per_sort, consecutive_transitions_per_sort)
            locm2_time = time.time() - start - polocm_time
        

            TS_overall, ap_state_pointers, OS = POLOCMBASELINE._step1(obj_consecutive_transitions_overall, sorts, S, AML, debug['step1'])
            HS = POLOCMBASELINE._step3(TS_overall, ap_state_pointers, OS, sorts, AML, debug["step3"])
            del TS_overall
            bindings = POLOCMBASELINE._step4(HS, debug["step4"])

            bindings = POLOCMBASELINE._step5(HS, bindings,ap_state_pointers, OS, debug["step5"])
            del HS
         
            fluents, actions = POLOCMBASELINE._step7(
                OS,
                ap_state_pointers,
                dependencies,
                sorts,
        ...
        return model, AML, (polocm_time, locm2_time, locm_time)
    ...
    def polocm_step1(
        ...
        """
        # TODO: add zero obj
        zero_obj = POLOCMBASELINE.zero_obj
        ...
        for obs_PO_trace in obs_PO_tracelist:
            ...
            obj_PO_traces: Dict[PlanningObject, List[IAP]] = defaultdict(list)
    ...
    def polocm_step2(
        ...
        obj_consecutive_transitions_overall = []
                ...
                for i in range(len(PO_matrix)):
                    for j in range(len(PO_matrix)):
                        ...
                        if (PO_matrix.iloc[i,j] == 1):
                            ...
                            obj_consecutive_transitions[obj].append((PO_matrix.columns[i],PO_matrix.columns[j]))
            obj_consecutive_transitions_overall.append(obj_consecutive_transitions)
        

        return obj_consecutive_transitions_overall

    @staticmethod
    def _locm2_step0(
        obj_consecutive_transitions_overall, sorts: Sorts, debug: bool = False
    ):
        """
            ...
            for obj, transitions in obj_consecutive_transitions.items():
                ...
                for iap1, iap2 in transitions:
                    ...
                    if (graphs[sort].has_edge(iap1.toAP(), iap2.toAP())):
                        ...
                        graphs[sort].add_edge(iap1.toAP(),iap2.toAP(),weight=1)
    ...
    @staticmethod
    def _step1(
        obj_consecutive_transitions_overall, sorts: Sorts, transition_sets_per_sort, AML, debug: bool = False
        ...
        for obj_consecutive_transitions in obj_consecutive_transitions_overall:
            ...
            for obj, consecutive_transitions in obj_consecutive_transitions.items():
                sort = sorts[obj.name]
                if obj != zero_obj:
                    transition_sets = transition_sets_per_sort[sort]
                    ...
                    for iap1, iap2 in consecutive_transitions:
                        ...
                        if (not TS[zero_fsm].get(zero_obj)):
                            ...
                            TS[zero_fsm][zero_obj].append(subseq)
        ...
        for fsm, ap_states in ap_state_pointers.items():
            for ap, state in ap_states.items():
                ...
                for prev_ap, val in prev_aps.items():
                    if val > 0:
                        current_start, _ = POLOCMBASELINE._pointer_to_set(OS[fsm], state.start, state.end)
                        prev_state = ap_state_pointers[fsm][prev_ap]
                        _, prev_end = POLOCMBASELINE._pointer_to_set(OS[fsm], prev_state.start, prev_state.end)
                        ...
                        for i, Bk_ in enumerate(B.action.obj_params):
                            ...
                            for j, Cl_ in enumerate(C.action.obj_params):
                                ...
                                if sorts[Cl_.name] == G_:
                                    ...
                                    S, S2 = POLOCMBASELINE._pointer_to_set(
                                        OS[fsm],
    ...
    def _step4(
        ...
        for fsm, hs_fsm in HS.items():
            for state, hs_fsm_state in hs_fsm.items():
                ...
                for i, h1 in enumerate(hs_fsm_state):
                    for h2 in hs_fsm_state[i + 1 :]:
                        ...
                        if (
                            ...
                            P1, P2 = POLOCMBASELINE._pointer_to_set(state_params, v1, v2)
                ...
                bindings[fsm][state] = [
                    Binding(h, POLOCMBASELINE._pointer_to_set(state_params, v)[0])
    ...
    def get_state_machines(
        ...
        for fsm in OS:
            ...
            for ap, apstate in ap_state_pointers[fsm].items():
                start_idx, end_idx = POLOCMBASELINE._pointer_to_set(
    ...
    def _step7(
        ...
        for (fsm, aps), states in zip(ap_state_pointers.items(), OS.values()):
            for ap, pointers in aps.items():
                start_state, end_state = POLOCMBASELINE._pointer_to_set(
